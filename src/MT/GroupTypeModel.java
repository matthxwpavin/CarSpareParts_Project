 ...
 * @author pwann
 */
public class GroupTypeModel extends AbstractTableModel {

    private int maxRowIndex = -1;
    private int numRow = 2000;
    private int numCol = 3;
    private String[] columnNames = {"รหัสหมวดหมู่", "ชื่อหมวดหมู่"};
    private Object[][] data = new Object[numRow][numCol];
    private ArrayList<PartGroup> dataList;

    public GroupTypeModel() {
        dataList = new ArrayList<PartGroup>();
    }

    public void addAllData(ArrayList<PartGroup> obj) {

        dataList = obj;

        for (PartGroup c : dataList) {

            int row = ++maxRowIndex;
            setValueAt(c.getID(), row, 0);
            setValueAt(c.getName(), row, 1);
